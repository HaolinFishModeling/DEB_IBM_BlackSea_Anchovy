# 20250305 D-D mortality for 2D
natural_and_DD_mortality_2D<-function(input.DD.mortality,
                                      results.each.age){
  
  if(input.DD.mortality==1){  # add D-D mortality only for Late-larvae
    
    Cal_ZFAC_LL<-function(Dt,Db=Db_LL,L){
      ZFAC<--1+2*Dt/Db+0.4*L-0.4*(Dt/Db)*L
      ZFAC <- ifelse(ZFAC < 0, 0, ZFAC)
      return(ZFAC)
    }
    
    Cal_ZFAC_juv<-function(Dt,Db=Db_juv,L){
      ZFAC<-1
      return(ZFAC)
    }
    ...
    results.each.age<-results.each.age %>%
      ...
      mutate(ZFAC = case_when(
        ...
        running_status=='alive' & life_stage == 'late_larvae' ~ Cal_ZFAC_LL(Dt=temporal_worth/(2780*(2780*cos(Lat*pi/180)))/30, L=TL),
        running_status=='alive' & life_stage == 'juvenile' ~ Cal_ZFAC_juv(Dt=temporal_worth/(2780*(2780*cos(Lat*pi/180)))/100, L=TL),
      ...
      mutate(density = case_when(
        running_status=='alive' & life_stage == 'egg' ~ worth/(2780*(2780*cos(Lat*pi/180)))/10,
        running_status=='alive' & life_stage == 'yolksac_larvae' ~ worth/(2780*(2780*cos(Lat*pi/180)))/10,
        running_status=='alive' & life_stage == 'early_larvae' ~ worth/(2780*(2780*cos(Lat*pi/180)))/30,
        running_status=='alive' & life_stage == 'late_larvae' ~ worth/(2780*(2780*cos(Lat*pi/180)))/30,
        running_status=='alive' & life_stage == 'juvenile' ~ worth/(2780*(2780*cos(Lat*pi/180)))/100,
        running_status=='alive' & life_stage == 'adult' ~ worth/(2780*(2780*cos(Lat*pi/180)))/100,
        TRUE ~ density
      ))%>%
      mutate(norm_density = case_when(
        ...
        running_status=='alive' & life_stage == 'late_larvae' ~ density/Db_LL,
    ...
    results.each.age<-results.each.age %>%
      ...
      # need to add normlized density and density before adding mortality
      ...
      mutate(norm_density = case_when(
        ...
        running_status=='alive' & life_stage == 'juvenile' ~ density/Db_juv,
        running_status=='alive' & life_stage == 'adult' ~ NA_real_,
        TRUE ~ norm_density
      ))%>%
      select(-temporal_worth)%>%
